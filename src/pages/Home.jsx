...
import { SolidCard, Stars, SolidBadge, SolidButton } from '../components/ui-custom';
...
export function Home() {
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 pb-20">
      {/* 1. Hero Section */}
      <section className="border-b-2 border-zinc-900 bg-zinc-50 pt-12 pb-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-sm bg-black text-white text-xs font-bold uppercase tracking-wider mb-6"
          >
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          {filteredShops.length === 0 ? (
            <div className="text-center py-16 p-8 rounded-md bg-zinc-50 border border-zinc-200">
              ...
              <h3 className="text-xl font-bold text-zinc-900">No repair shops match your query</h3>
      ...
      {!query && activeCategory === 'All' && (
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
          {Array.from(groupedShops.entries()).map(([catKey, shopList], idx) => {
            ...
            return (
              <div key={catKey} className="border-b border-zinc-200 pb-10 last:border-b-0">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="rounded-md bg-zinc-950 text-white p-8 sm:p-12 border-2 border-black">
          <div className="max-w-2xl mb-8">
            <SolidBadge variant="yellow" className="mb-3">
              Why Fixly?
            </SolidBadge>
...
function CategoryChip({ label, emoji, color, active, onClick }) {
  return (
    <motion.button
      type="button"
      whileTap={{ scale: 0.95 }}
...
function ShopCard({ shop, index, onOpen }) {
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="flex flex-col h-full rounded-md bg-white border-2 border-zinc-900 shadow-sm hover:shadow-md transition-shadow overflow-hidden text-left"
    >
      {/* Header banner */}
      <div className="p-5 border-b border-zinc-200 bg-zinc-50 flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-lg font-black text-zinc-950 truncate">{shop.name}</h3>
            <p className="text-xs text-zinc-500 font-bold uppercase tracking-wider truncate mt-0.5">
              {shop.owner}
            </p>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-1">
          {shop.categories.slice(0, 3).map((cat) => (
            <SolidBadge key={cat} variant="neutral" className="text-[10px] py-0.5">
              {cat}
            </SolidBadge>
          ))}
          {shop.categories.length > 3 && (
            <SolidBadge variant="purple" className="text-[10px] py-0.5">
        ...
        <SolidButton
          ...
          size="sm"
          onClick={onOpen}
          className="w-full py-2 text-xs font-bold uppercase tracking-wider mt-1"
        >
...
function ValueCard({ icon, color, title, desc }) {
  return (
    <div className="p-4 rounded-md bg-zinc-900 border border-zinc-800 flex flex-col gap-2">
